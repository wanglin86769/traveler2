...
const binder = new Schema({
  ...
  publicAccess: {
    type: Number,
    default: config.defaults.binder_public_access,
  },
...
binder.methods.updateProgress = async function(cb) {
  const { works } = this;
  let totalValue = 0;
  let finishedValue = 0;
  ...
  if (this.isModified()) {
    ...
    try {
      const newBinder = await this.save();
      if (cb) {
        cb(null, newBinder);
      }
    } catch (err) {
 ...
 * @return {undefined}
 */
async function updateParentProgress(child) {
  try {
    ...
    logger.error(
      `cannot find binders for traveler ${child._id}, error: ${error.message}`
    );
  }
}
