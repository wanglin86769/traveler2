...
const TravelerTransferOwnershipDialog = ({
  open,
  onClose,
  onConfirm,
  selectedTravelers,
  ...
  return (
    <Dialog open={open} onClose={handleCancel} maxWidth="sm" fullWidth>
      <DialogTitle>
        Transfer the following {selectedTravelers.length} traveler{selectedTravelers.length > 1 ? 's' : ''}?
      </DialogTitle>
      ...
      <DialogContent>
        ...
        <List>
          {selectedTravelers.map(traveler => (
            <ListItem key={traveler.id || traveler._id} sx={{ py: 0.5 }}>
              <ListItemText 
                primary={traveler.title} 
              />
            </ListItem>
          ))}
        </List>
...
export default TravelerTransferOwnershipDialog